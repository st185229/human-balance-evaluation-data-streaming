...
customerDetailsStruct = pyspark.sql.types.StructType(
    [
        pyspark.sql.types.StructField("customer", pyspark.sql.types.StringType()),
        pyspark.sql.types.StructField("score", pyspark.sql.types.FloatType()),
        pyspark.sql.types.StructField("riskDate", pyspark.sql.types.DateType())
    ]
)

stediEventsDF.withColumn("value", from_json("value", customerDetailsStruct)) \
...
# +--------------------+-----
customerRiskDF.writeStream.outputMode("append").format("console").start().awaitTermination()
