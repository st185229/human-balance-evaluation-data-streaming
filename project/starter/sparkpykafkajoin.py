...
# Create a StructType for the Kafka redis-server topic which has all changes made to Redis - before Spark
# 3.0.0, schema inference is not automatic


radisServerTopicStructure = StructType(
    [
...
#  create a StructType for the Customer JSON that comes from Redis- before Spark 3.0.0, schema inference is not
# automatic

radisCustomerJsonSchema = StructType(
    [
...
radisStediEventTopicStructure = StructType(
    [
        StructField("customer", StringType()),
...
spark = SparkSession.builder.appName("stedi-radis-object").getOrCreate()

# TO-DO: set the spark log level to WARN

spark.sparkContext.setLogLevel('WARN')

# TO-DO: using the spark application object, read a streaming dataframe from the Kafka topic redis-server as the
...
kafkaRedisDF.withColumn("value", from_json("value", radisServerTopicStructure)) \
...
zSetEntriesEncodedCustomerFrame = spark.sql("select zSetEntries[0].element as encodedCustomer from RedisSortedSet")
...
zSetDecodedEntriesStreamingDF = zSetEntriesEncodedCustomerFrame.withColumn("customer", unbase64(
    zSetEntriesEncodedCustomerFrame.encodedCustomer).cast("string"))
...
zSetDecodedEntriesStreamingDF.withColumn("customer", from_json("customer", customer)) \
    .select(col('customer.*')) \
    .createOrReplaceTempView("CustomerRecords")
# TO-DO: JSON parsing will set non-existent fields to null, so let's select just the fields we want, where they are
...
kafkaEventsDF.withColumn("value", from_json("value", radisStediEventTopicStructure)) \
...
# TO-DO: join the streaming dataframes on the email address to get the risk score and the birth year in the same
# dataframe
